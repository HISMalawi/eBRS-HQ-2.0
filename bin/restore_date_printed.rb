...
births.each_with_index do |birth, i|
  ...
  if !birth.source_id.blank? && birth.source_id.length > 19
    ...
    puts "R--#{date}##{date_issued}"
  elsif birth.source_id.blank? || birth.source_id.length <= 19
    local_date = PersonRecordStatus.where(" status_id IN (#{print_statuses.join(',')}) ").first
    local_issue_date = PersonRecordStatus.where(" status_id IN (#{dispatch_statuses.join(',')}) ").first

    date          = local_date.created_at.to_datetime rescue nil
    date_issued   = local_issue_date.to_datetime rescue nil
    puts "L--#{date}##{date_issued}"
  end
